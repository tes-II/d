...
class Auth:
    ...
    #         "id_token": str
    #     },
    #     "name": "optional friendly name from profile"
    # }
    ...
    def add_refresh_token(self, number: int, refresh_token: str, name: str = ""):
        ...
        if existing:
            ...
            # Attempt to extract friendly name from profile response (common keys)
            profile_name = profile_data["profile"].get("name") or profile_data["profile"].get("display_name") or profile_data["profile"].get("caller_name") or name or ""
    ...
    def set_active_user(self, number: int):
        ...
        # Attempt to read a friendly display name from profile
        profile_name = profile_data["profile"].get("name") or profile_data["profile"].get("display_name") or profile_data["profile"].get("caller_name") or rt_entry.get("name", "")
        ...
        # Update refresh token entry with subscriber_id and subscription_type and name
        rt_entry["subscriber_id"] = subscriber_id
        rt_entry["subscription_type"] = subscription_type
        if profile_name and profile_name.strip() != "":
            rt_entry["name"] = profile_name.strip()
    ...
    def renew_active_user_token(self):
        if self.active_user:
            ...
            if tokens:
                ...
                self.last_refresh_time = int(time.time())
                # update stored refresh token as well
